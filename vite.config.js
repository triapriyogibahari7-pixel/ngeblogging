...
export default defineConfig({
  plugins: [
    {
      name: "ngeblogging-diagnostic-v259-finalizer-only",
      apply: "build",
      async buildStart() {
        const v250 = await activateStudioNativeV250();
        console.log(`[vite] ${v250.release}`);
        const v256 = await finalizeStudioV255Order();
        console.log(`[vite] ${v256.release}`);
        const v257 = await finalizeStudioV257Order();
        console.log(`[vite] ${v257.release}`);
        const v259 = await finalizeStudioV259Order();
        console.log(`[vite] ${v259.release}`);
      },
      closeBundle() {
