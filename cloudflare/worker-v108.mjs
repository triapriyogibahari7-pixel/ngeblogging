...
const RELEASE = "2026.07.28-auth-gateway-v108";
...
function json(status, payload, requestId) {
  return new Response(JSON.stringify({ ...payload, release: RELEASE, requestId }), {
    status,
    headers: {
      "content-type": "application/json; charset=utf-8",
      "cache-control": "no-store",
      "x-content-type-options": "nosniff",
      "x-ngeblogging-auth-gateway": RELEASE,
...
async function handleAuthProxy(request, env, requestId) {
  ...
  try {
    ...
    responseHeaders.set("x-ngeblogging-auth-gateway", RELEASE);
...
export default {
  async fetch(request, env, context) {
    ...
    if (url.pathname.startsWith(`${PREFIX}/auth/v1/`)) {
      return handleAuthProxy(request, env, crypto.randomUUID());
    }
