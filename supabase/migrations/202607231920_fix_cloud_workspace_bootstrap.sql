...
-- Remove the obsolete quota trigger that allowed 25 Pro sites. The authoritative
-- enforce_owned_site_limit trigger already applies 5 sites on Free and 12 total
-- on every non-Free account with an advisory transaction lock.
drop trigger if exists sites_enforce_account_quota on public.sites;
drop function if exists private.enforce_site_account_quota();
...
begin
  ...
  if char_length(normalized_slug) not between 3 and 63
     ...
     or not public.is_site_slug_available(normalized_slug, null)
  then
    raise exception using errcode = '23505', message = 'Subdomain tidak tersedia atau formatnya tidak valid.';
  end if;
...
comment on function public.create_site_workspace(text,text,text,text) is
  'Atomically creates an authenticated user workspace, owner membership, and managed *.ngeblogging.com domain while enforcing the authoritative 5/12 account quota.';
