-- Keep site creation under the caller's Row Level Security policies.
-- The owner-membership and managed-subdomain triggers remain SECURITY DEFINER
-- in the private schema, while the exposed RPC no longer bypasses RLS.
...
begin
  ...
  allowed_limit := case when lower(coalesce(account_plan, 'free')) = 'free' then 5 else 12 end;
...
revoke all on function public.create_site_workspace(text,text,text,text) from public;
revoke all on function public.create_site_workspace(text,text,text,text) from anon;
grant execute on function public.create_site_workspace(text,text,text,text) to authenticated;
grant execute on function public.create_site_workspace(text,text,text,text) to service_role;
