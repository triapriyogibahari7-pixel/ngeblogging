...
alter table public.sites
  add column if not exists theme text not null default 'Editorial Pro',
  add column if not exists layout text not null default 'editorial',
  add column if not exists custom_domain text;

update public.sites
set theme = coalesce(nullif(theme, ''), nullif(theme_key, ''), 'Editorial Pro'),
    layout = coalesce(nullif(layout, ''), nullif(settings->>'layout', ''), 'editorial'),
    custom_domain = coalesce(custom_domain, nullif(settings->>'customDomain', ''));
...
with check (
  ...
  and private.has_site_role(
    ...
    array['owner'::public.member_role,'admin'::public.member_role,'editor'::public.member_role,'author'::public.member_role,'contributor'::public.member_role]
  )
);
