...
as $function$
  select coalesce(
    (select capacity.allowed_limit from private.account_site_capacity capacity where capacity.user_id = target_user),
...
begin
  ...
  select coalesce(profile.plan, 'free') into account_plan
    from public.profiles profile where profile.id = new.owner_id;

  allowed_count := private.site_limit_for_owner(new.owner_id, account_plan);

  select count(*)::integer into current_count
    from public.sites site where site.owner_id = new.owner_id;
...
as $function$
  with account as (
    ...
    select state.*, private.site_limit_for_owner(state.user_id, state.plan) as allowed_limit
    from state where state.user_id is not null
  )
  select
