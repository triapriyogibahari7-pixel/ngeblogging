...
for (const marker of ["handleNaraImage", "imageGenerationReady", "workersVisionReady", "handleBillingRequest", "handlePayPalWebhook", "seoEndpoint", "injectTenantSeo", "handleWorkersAiNara", "/api/nara/image", "/api/billing/paypal/webhook", "/api/billing/", "DEFAULT_RELEASE"]) if (!worker.includes(marker)) throw new Error(`Worker belum memuat kemampuan produksi: ${marker}`);
...
for (const marker of ["studio-v14-authority.css", "nara-interaction-authority.css", "nara-command-center-bridge.js", "nara-command-center.css"]) if (!index.includes(marker)) throw new Error(`Shell produksi belum memuat ${marker}.`);
...
for (const marker of ["studio-source-navigation-v14-20260724", "data.naraWorkspaceRoute", ".sn-top-actions .sn-nara-button", "data.sidebarAuthority", "PHONE_QUERY", "health.imageGeneration"]) {
...
for (const marker of [".nara-floating-button", ".nara-assistant-layer", ".nara-assistant-shell", ".nara-native-file-input", "pointer-events: auto"]) {
...
for (const marker of ["npm run deploy:cloudflare", "studio-v14-authority.css", "nara-interaction-authority.css", "nara-command-center-bridge.js", "ngeblogging-app-v14-20260724", "2026.07.24-studio-v14", "health.naraProviders?.vision", "health.imageGeneration", "TENANT_SMOKE_TEST_URL", "tenant-404"]) {
