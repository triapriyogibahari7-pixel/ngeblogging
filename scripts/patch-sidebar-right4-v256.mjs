...
async function verify() {
  ...
  for (const marker of [
    ...
    'id: "sidebar-right-4"',
    MARKER,
  ]) if (!widgets.includes(marker)) throw new Error(`V256_RIGHT4_WIDGET_VERIFY_MISSING:${marker}`);

  if (!runtime.includes('const LEFT_AREAS = ["sidebar-left-1", "sidebar-left-2", "sidebar-left-3", "sidebar-left-4"];')) throw new Error("V256_LEFT4_RUNTIME_LOST");
  if (!runtime.includes('const RIGHT_AREAS = ["sidebar-right-1", "sidebar-right-2", "sidebar-right-3", "sidebar-right-4"];')) throw new Error("V256_RIGHT4_RUNTIME_MISSING");
