...
const WORKER_RELEASE = "2026.07.31-mobile-auth-interaction-v178";
const read = (file) => fs.readFileSync(path.join(root, file), "utf8");
const write = (file, content) => fs.writeFileSync(path.join(root, file), content);

function replaceRequired(source, search, replacement, label) {
  if (source.includes(replacement)) return source;
  if (!source.includes(search)) throw new Error(`Patch v178 gagal: ${label} tidak ditemukan.`);
...
function patchStudioEntry() {
  ...
  if (!source.includes('import "./studio-interaction-v178.js";')) {
    source = replaceRequired(
      source,
      "\n\nexport default StudioFastGate;",
      '\nimport "./studio-interaction-v178.js";\n\nexport default StudioFastGate;',
      "entry v178",
    );
  }
  write(file, source);
...
function patchAuthTransport() {
  ...
  source = replaceRequired(
    ...
    'async function authAwareFetch(input, init) {\n  if (!nativeFetch) throw new Error("Fetch API tidak tersedia pada browser ini.");\n  const proxy = proxiedAuthUrl(input);\n  if (!proxy) return nativeFetch(input, init);\n\n  const direct = input instanceof Request ? input.clone() : input;\n  const request = input instanceof Request\n    ? new Request(proxy.toString(), input)\n    : proxy.toString();\n  try {\n    const response = await nativeFetch(request, init);\n    const retryDirect = [502, 503, 504].includes(response.status);\n    if (typeof document !== "undefined") {\n      document.documentElement.dataset.authTransportV178 = retryDirect ? "gateway-retry-direct" : "same-origin-gateway";\n      document.documentElement.dataset.authTransportV153 = response.headers.get("x-ngeblogging-auth-gateway")\n        ? "same-origin-gateway"\n        : "same-origin-response";\n    }\n    if (!retryDirect) return response;\n    return nativeFetch(direct, init);\n  } catch (error) {\n    if (typeof document !== "undefined") document.documentElement.dataset.authTransportV178 = "network-retry-direct";\n    return nativeFetch(direct, init);\n  }\n}',
    "auth gateway direct fallback",
  );
  source = replaceRequired(
    source,
    ...
    'function providerDestination(value) {\n  const direct = new URL(value);\n  if (direct.origin === supabaseOrigin() && direct.pathname.startsWith("/auth/v1/authorize")) {\n    if (typeof document !== "undefined") document.documentElement.dataset.authProviderTransportV178 = "direct-supabase-authorize";\n    return direct.toString();\n  }\n  const proxy = proxiedAuthUrl(direct.toString());\n  return proxy?.toString() || direct.toString();\n}',
    "OAuth authorize direct",
  );
  write(file, source);
}
...
function patchServiceWorker() {
  ...
  let source = read(file);
  if (source.includes('const VERSION = "ngeblogging-app-v178-mobile-auth-interaction-20260731";')) return;
  source = replaceRequired(
    source,
    'const VERSION = "ngeblogging-app-v177-screenshot-stability-20260731";',
    'const VERSION = "ngeblogging-app-v178-mobile-auth-interaction-20260731";\nconst SCREENSHOT_STABILITY_COMPAT_VERSION = "ngeblogging-app-v177-screenshot-stability-20260731";',
    "SW version v177",
  );
  source = replaceRequired(
    source,
    'const CACHE_RELEASE = "screenshot-stability-cache-v177";',
    'const CACHE_RELEASE = "mobile-auth-interaction-cache-v178";\nconst SCREENSHOT_STABILITY_COMPAT_RELEASE = "screenshot-stability-cache-v177";',
    "SW cache v177",
  );
  source = replaceRequired(
    source,
    'const FORCE_REFRESH_VALUE = "screenshot-stability-v177";',
    'const FORCE_REFRESH_VALUE = "mobile-auth-interaction-v178";\nconst SCREENSHOT_STABILITY_COMPAT_FORCE_REFRESH = "screenshot-stability-v177";',
    "SW refresh v177",
  );
  source = source.replaceAll("NGE_BLOGGING_FORCE_RELOAD_V177", "NGE_BLOGGING_FORCE_RELOAD_V178");
  source = source.replaceAll("service-worker-stale-shell-v177", "service-worker-stale-shell-v178");
  source = source.replaceAll("service-worker-activated-screenshot-stability-v177", "service-worker-activated-mobile-auth-interaction-v178");
  source = replaceRequired(
    source,
    '    screenshotStabilityRelease: "studio-screenshot-stability-v177-20260731",',
    '    screenshotStabilityRelease: "studio-screenshot-stability-v177-20260731",\n    mobileAuthInteractionRelease: "studio-mobile-auth-interaction-v178-20260731",\n    screenshotStabilityCompatVersion: SCREENSHOT_STABILITY_COMPAT_VERSION,\n    screenshotStabilityCompatRelease: SCREENSHOT_STABILITY_COMPAT_RELEASE,\n    screenshotStabilityCompatForceRefresh: SCREENSHOT_STABILITY_COMPAT_FORCE_REFRESH,\n    mobileAuthInteractionCompatibility: ["NGE_BLOGGING_FORCE_RELOAD_V177", "service-worker-stale-shell-v177", "service-worker-activated-screenshot-stability-v177"],',
    "SW payload v177",
  );
  write(file, source);
}
...
function patchWorker() {
  ...
  source = replaceRequired(
    source,
    'export const SCREENSHOT_STABILITY_RELEASE = "2026.07.31-screenshot-stability-v177";',
    `export const SCREENSHOT_STABILITY_RELEASE = "2026.07.31-screenshot-stability-v177";\nexport const MOBILE_AUTH_INTERACTION_RELEASE = "${WORKER_RELEASE}";`,
    "worker v177 release",
  );
  source = replaceRequired(
    source,
    '  "/release-v177.json",\n]);',
    '  "/release-v177.json",\n  "/release-v178.json",\n]);',
    "worker release path v178",
  );
  source = replaceRequired(
    source,
    '    screenshotStabilityRelease: SCREENSHOT_STABILITY_RELEASE,',
    '    screenshotStabilityRelease: SCREENSHOT_STABILITY_RELEASE,\n    mobileAuthInteractionRelease: MOBILE_AUTH_INTERACTION_RELEASE,\n    drawerBackdropOutsideOnly: true,\n    naraSmallMediumNonmodalV178: true,\n    profileSeparatedFromSettings: true,\n    oauthAuthorizeDirectTransport: true,',
    "worker release body v178",
  );
  source = replaceRequired(
    source,
    '      "x-ngeblogging-screenshot-stability": SCREENSHOT_STABILITY_RELEASE,',
    '      "x-ngeblogging-screenshot-stability": SCREENSHOT_STABILITY_RELEASE,\n      "x-ngeblogging-mobile-auth-interaction": MOBILE_AUTH_INTERACTION_RELEASE,',
    "worker release header v178",
  );
  source = replaceRequired(
    source,
    '    && html.includes("ngeblogging-screenshot-stability-v177")',
    '    && html.includes("ngeblogging-screenshot-stability-v177")\n    && html.includes("ngeblogging-mobile-auth-interaction-v178")',
    "worker marker condition v178",
  );
  source = replaceRequired(
    source,
    '    `<meta name="ngeblogging-screenshot-stability-v177" content="${SCREENSHOT_STABILITY_RELEASE}"/>`,',
    '    `<meta name="ngeblogging-screenshot-stability-v177" content="${SCREENSHOT_STABILITY_RELEASE}"/>`,\n    `<meta name="ngeblogging-mobile-auth-interaction-v178" content="${MOBILE_AUTH_INTERACTION_RELEASE}"/>`,',
    "worker marker v178",
  );
...
function patchNetlify() {
  ...
  source = replaceRequired(
    source,
    'const SCREENSHOT_STABILITY_RELEASE = "2026.07.31-screenshot-stability-v177";',
    `const SCREENSHOT_STABILITY_RELEASE = "2026.07.31-screenshot-stability-v177";\nconst MOBILE_AUTH_INTERACTION_RELEASE = "${WORKER_RELEASE}";`,
    "Netlify v177 release",
  );
  source = replaceRequired(
    source,
    '  X-Ngeblogging-Screenshot-Stability: ${SCREENSHOT_STABILITY_RELEASE}',
  ...
  source = replaceRequired(
    ...
    '/release-v177.json\n  Cache-Control: no-store, max-age=0\n/release-v178.json\n  Cache-Control: no-store, max-age=0\n`,',
    "Netlify release path v178",
  );
  source = replaceRequired(
    source,
    '    screenshotStabilityRelease: SCREENSHOT_STABILITY_RELEASE,',
    '    screenshotStabilityRelease: SCREENSHOT_STABILITY_RELEASE,\n    mobileAuthInteractionRelease: MOBILE_AUTH_INTERACTION_RELEASE,\n    drawerBackdropOutsideOnly: true,\n    naraSmallMediumNonmodalV178: true,\n    profileSeparatedFromSettings: true,\n    oauthAuthorizeDirectTransport: true,',
    "Netlify release body v178",
  );
  source = replaceRequired(
    source,
    '    || !html.includes(\'name="ngeblogging-screenshot-stability-v177"\')',
    '    || !html.includes(\'name="ngeblogging-screenshot-stability-v177"\')\n    || !html.includes(\'name="ngeblogging-mobile-auth-interaction-v178"\')',
    "Netlify marker condition v178",
  );
  source = replaceRequired(
    source,
    '      `<meta name="ngeblogging-screenshot-stability-v177" content="${SCREENSHOT_STABILITY_RELEASE}">`,',
...
function verify() {
  const checks = [
    ["src/Studio.jsx", 'import "./studio-interaction-v178.js";'],
    ["src/studio-interaction-v178.js", RELEASE],
    ["src/studio-interaction-v178.css", "--sm178-drawer-width"],
    ["src/studio-interaction-v178.css", 'data-nara-interaction-v178="nonmodal"'],
    ...
    ["public/sw.js", "ngeblogging-app-v178-mobile-auth-interaction-20260731"],
    ["public/sw.js", "mobile-auth-interaction-cache-v178"],
    ["public/sw.js", 'url.pathname === "/login"'],
    ["cloudflare/worker-v69.mjs", '"/release-v178.json"'],
    ["cloudflare/worker-v69.mjs", "x-ngeblogging-mobile-auth-interaction"],
    ["scripts/write-netlify-redirects.mjs", "/release-v178.json"],
  ...
  if (missing.length) throw new Error(`Patch v178 tidak lengkap: ${missing.map(([file, marker]) => `${file}:${marker}`).join(", ")}`);
}
