...
const SW_VERSION = "ngeblogging-app-v205-theme-nara-auth-mobile-20260802";
const SW_CACHE = "theme-nara-auth-mobile-cache-v205";
const SW_REFRESH = "theme-nara-auth-mobile-v205";
...
async function patchServiceWorker() {
  ...
  source = source.replace(/\n\s*await refreshStaleWindow\(client, url\);/g, "\n      // v205: notify only; login session, callback and unsaved drafts are never force-navigated.");
...
async function verify() {
  const checks = [
    ...
    ["src/studio-production-v205.js", "normalizeTheme"],
    ["src/studio-production-v205.js", "normalizeNara"],
    ["src/studio-production-v205.js", "normalizeStartup"],
    ["src/studio-production-v205.css", 'data-studio-mobile-v205="true"'],
    ["src/studio-production-v205.css", ".tn-layout-canvas-v170"],
    ["src/studio-production-v205.css", ".tn-code-workspace"],
  ...
  if (/localStorage\.clear\s*\(|sessionStorage\.clear\s*\(|signOut\s*\(/.test(runtime)) {
    throw new Error("V205_RUNTIME_SESSION_DESTRUCTIVE_ACTION_FOUND");
  }
  ...
  if (release.validation?.googleLoginEndToEndClaimed !== false) throw new Error("V205_UNSUPPORTED_GOOGLE_CLAIM");
  if (release.validation?.linkedinLoginEndToEndClaimed !== false) throw new Error("V205_UNSUPPORTED_LINKEDIN_CLAIM");
  if (release.validation?.emailPasswordEndToEndClaimed !== false) throw new Error("V205_UNSUPPORTED_EMAIL_CLAIM");
}
