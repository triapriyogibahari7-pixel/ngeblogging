...
for (const marker of [RELEASE,"MAX_CODE_LINES = 10000","activateSingleToggle","event.stopImmediatePropagation()","normalizeNara","enhanceCodeEditor"]) {
  if (!runtime.includes(marker)) throw new Error(`V275_RUNTIME_MARKER_MISSING:${marker}`);
}
for (const marker of ["--v275-side-open:240px","#ngeblogging-studio-sidebar",".nara-floating-button",'data-nara-interaction="nonmodal"',".tn-layout-map-v264",".tn-code-workspace",".v275-code-lines",".sv124-free-domain"]) {
  if (!css.includes(marker)) throw new Error(`V275_CSS_MARKER_MISSING:${marker}`);
}
for (const marker of ['import "./studio-final-device-authority-v268.css";','import "./studio-profile-menu-v268.js";','import "./studio-nara-immediate-v268.css";','import "./studio-final-stability-v275.js";',"event.stopImmediatePropagation()"]) {
  if (!toggle.includes(marker)) throw new Error(`V275_TOGGLE_COMPAT_MARKER_MISSING:${marker}`);
}
for (const marker of ["one internal n remains the single sidebar interaction bridge","real line numbers up to 10000","auth persistence remains production-backed"]) {
  if (!tests.includes(marker)) throw new Error(`V275_TEST_MARKER_MISSING:${marker}`);
}
for (const value of [runtime, css, toggle]) {
  if (/localStorage\.clear\s*\(|sessionStorage\.clear\s*\(|location\.reload\s*\(|signOut\s*\(/.test(value)) throw new Error("V275_DESTRUCTIVE_SESSION_OR_RELOAD_ACTION");
}
...
source = source
  .replace(/^const SHELL_CACHE = .*;$/m, 'const SHELL_CACHE = `${ACTIVE_VERSION_V275}-${ACTIVE_CACHE_RELEASE_V275}-${STUDIO_FINAL_STABILITY_RELEASE_V275}-${UI_CACHE_RELEASE_V275}-${AUTH_HANDOFF_RELEASE}-shell`;')
  .replace(/^const ASSET_CACHE = .*;$/m, 'const ASSET_CACHE = `${ACTIVE_VERSION_V275}-${ACTIVE_CACHE_RELEASE_V275}-${STUDIO_FINAL_STABILITY_RELEASE_V275}-${UI_CACHE_RELEASE_V275}-${AUTH_HANDOFF_RELEASE}-assets`;')
...
if (!source.includes(RELEASE) || !source.includes(CACHE_RELEASE)) throw new Error("V275_SW_MARKER_MISSING");
...
await import("./patch-service-worker-v276.mjs");
