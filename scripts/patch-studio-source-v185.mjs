...
export function getActiveSiteSnapshot() {
  if (typeof window !== "undefined" && window.__ngebloggingActiveSite?.id) {
    return window.__ngebloggingActiveSite;
  }
  try {
...
export function setActiveSiteSnapshot(site) {
  ...
  try {
    ...
    // Snapshot hanya jalur pemulihan; kegagalan storage tidak boleh memblokir Studio.
  }
  return snapshot;
}
...
function patchStudioNext() {
  ...
  if (!source.includes("studio-bootstrap-primary-first-v185")) {
    const startMarker = '  useEffect(() => {\n    if (!user?.id || !supabaseConfigured) { setDataMode("local"); return; }';
    const nextMarker = '\n\n  useEffect(() => {\n    if (dataMode !== "cloud"';
    const start = source.indexOf(startMarker);
    const end = source.indexOf(nextMarker, start);
    if (start < 0 || end < 0) throw new Error("PATCH_V185_STUDIO_BOOTSTRAP_ANCHOR_MISSING");
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const snapshot = setActiveSiteSnapshot(primary) || primary;
        setSite(snapshot);
        setSites((current) => current.some((item) => item.id === snapshot.id) ? current : [snapshot, ...current]);
        setDataMode("cloud");
      } catch (error) {
        ...
        if (!cancelled) {
          if (cached) {
            setDataMode("cloud");
            setToast("Koneksi terbatas: situs aktif dipulihkan dari perangkat dan sesi tetap tersimpan");
          } else {
            setDataMode("local");
            setToast("Cloud belum dapat dijangkau; draf perangkat tetap tersedia");
          }
        }
      }
  ...
  if (!source.includes("ngeblogging:active-site-ready")) {
    const marker = '\n\n  useEffect(() => {\n    if (dataMode !== "cloud"';
  ...
  source = replaceOnce(
    ...
    '  const selectSite = (next) => { const snapshot = setActiveSiteSnapshot(next) || next; setActiveSiteId(snapshot.id); setSite(snapshot); setSiteManager(false); setDocs([]); setView("home"); setToast(`Workspace ${snapshot.name} aktif`); window.dispatchEvent(new CustomEvent("ngeblogging:active-site-change", { detail: snapshot })); };',
    "STUDIO_SELECT_SITE",
  );
  write(file, source);
}
...
function patchEntryAndCache() {
  ...
  let sw = read(swFile);
  sw = sw.replace(/const VERSION = "[^"]+";/, 'const VERSION = "ngeblogging-app-v185-source-recovery-20260801";');
  sw = sw.replace(/const CACHE_RELEASE = "[^"]+";/, 'const CACHE_RELEASE = "source-recovery-cache-v185";');
  sw = sw.replace(/const FORCE_REFRESH_VALUE = "[^"]+";/, 'const FORCE_REFRESH_VALUE = "source-recovery-v185";');
  write(swFile, sw);
}
...
patchEntryAndCache();
verify();
console.log(`[${RELEASE}] source recovery, active-site cache, auth fallback, and loading termination verified`);
