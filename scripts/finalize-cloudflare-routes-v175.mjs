...
const LEGACY_EXACT_ROUTE_PATTERNS = new Set(["ngeblogging.com/*", "www.ngeblogging.com/*"]);
...
async function verifyZone() {
  ...
  if (zone?.id !== ZONE_ID || zone?.name !== ZONE_NAME || zone?.status !== "active") throw new Error("Zone aktif ngeblogging.com tidak cocok dengan CLOUDFLARE_ZONE_ID.");
  if (String(zone?.account?.id || "") !== ACCOUNT_ID) throw new Error("Zone ngeblogging.com tidak dimiliki CLOUDFLARE_ACCOUNT_ID deployment.");
}
...
async function deleteLegacyExactRoutes(routes) {
  ...
  for (const route of routes) {
    ...
    await request(`/zones/${encodeURIComponent(ZONE_ID)}/workers/routes/${encodeURIComponent(route.id)}`, { method: "DELETE" });
...
async function verifyFinalState() {
  ...
  for (const hostname of EXACT_HOSTNAMES) {
    ...
    if (String(domain?.service || "") !== SERVICE) throw new Error(`Worker Domain ${hostname} masih menunjuk ke service ${domain?.service || "unknown"}.`);
  }
  ...
  if (String(wildcard?.script || "") && String(wildcard.script) !== SERVICE) throw new Error(`Wildcard tenant menunjuk ke service tidak sesuai: ${wildcard.script}`);
...
console.log(JSON.stringify({
  status: "ok",
  release: PRODUCTION_LOGIN_FINALIZER_RELEASE,
  accountId: "[redacted]",
  zoneId: "[redacted]",
  zoneName: ZONE_NAME,
  attached,
