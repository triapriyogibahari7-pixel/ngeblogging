...
export async function finalizeStudioV259Order() {
  ...
  for (const required of [V257_RUNTIME, V257_STYLES, RUNTIME, STYLES, HOTFIX]) {
  ...
  source = removeLiveImport(source, HOTFIX);
  ...
  if (!(v257Runtime >= 0 && v257Styles > v257Runtime && runtime > v257Styles && styles > runtime && hotfix > styles)) {
    throw new Error("V259_FINAL_ORDER_INVALID");
  }
  for (const [path, code] of [[RUNTIME, "RUNTIME"], [STYLES, "CSS"], [HOTFIX, "HOTFIX"]]) {
    if ((source.match(new RegExp(escapeRegExp(`import "./${path}";`), "g")) || []).length !== 1) {
      throw new Error(`V259_${code}_DUPLICATE`);
    }
  }
