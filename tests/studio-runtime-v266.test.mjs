...
test("v266 is loaded after final v265 shell authority", () => {
  const finalV265 = studio.indexOf('import "./studio-shell-v265-final-hotfix.css";');
  const v266 = studio.indexOf('import "./studio-runtime-v266.js";');
  assert.ok(finalV265 >= 0);
  assert.ok(v266 > finalV265);
});

test("sidebar desktop state persists and small family is not forced into desktop state", () => {
...
test("Analytics is restored using real production RPC first", () => {
...
test("Theme Studio keeps eight device previews, 26-slot map and readable code gutters", () => {
...
test("Nara keeps camera photo microphone speaker model intelligence and nonmodal behavior", () => {
