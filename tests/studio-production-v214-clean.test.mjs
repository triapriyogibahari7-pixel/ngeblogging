...
const lateAuthPatch = read("scripts/patch-auth-late-callback-v215.mjs");
const worker = read("public/sw.js");
const release = JSON.parse(read("public/release-v214.json"));
const release215 = JSON.parse(read("public/release-v215.json"));

test("v214 keeps six responsive modes and large tablet desktop family while v215 auth remains present", () => {
  ...
  assert.match(lateAuthPatch, /auth-late-callback-recovery-v215-20260802/);
...
test("Theme editor remains 50:50 on large surfaces and preview-first on small surfaces", () => {
...
test("100 themes and 26 widgets remain intact including custom HTML JavaScript", () => {
...
test("Nara keeps small medium full non-modal small medium and all requested controls", () => {
...
test("Domain and factual analytics stay readable without synthetic production values", () => {
...
test("Profile Settings and Logout are distinct and Settings does not contain the Profile section", () => {
...
test("persistent session v215 recovery and explicit logout protections stay intact", () => {
...
test("release contracts stay factual and refuse unsupported mass-login/all-provider claims", () => {
  ...
  assert.equal(release.repairs.profileAndSettingsSurfacesSeparated, true);
