...
test("phone navigation is sidebar-only with one edge-mounted control", () => {
  ...
  assert.match(mobile, /--sn-phone-panel: min\(78vw, 300px\)/);
  assert.match(mobile, /\.sn-side:not\(\.collapsed\) \+ \.sn-main \.sn-icon[\s\S]*left: calc\(var\(--sn-phone-panel\) - 23px\)/);
  assert.match(mobile, /\.sn-side\.collapsed \+ \.sn-main \.sn-icon[\s\S]*left: 12px/);
...
test("settings and content lists cannot overlap on phones", () => {
  ...
  assert.match(mobile, /\.sn-settings-extras[\s\S]*grid-template-columns: minmax\(0, 1fr\)/);
  assert.match(mobile, /\.sn-content-tools[\s\S]*grid-template-columns: minmax\(0, 1fr\)/);
  assert.match(mobile, /\.sn-doc-row[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto 42px/);
  assert.match(guard, /window\.scrollTo\(\{ top: 0, left: 0/);
});

test("Nara has an independent Cloudflare Workers AI fallback", () => {
