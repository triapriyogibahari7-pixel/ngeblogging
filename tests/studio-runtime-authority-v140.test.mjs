...
test("production shell executes only React v140 as Studio layout runtime", () => {
...
test("login, PWA, Nara, quota, billing, favicon, and connectors stay active", () => {
  for (const runtime of activeFeatureRuntimes) {
    assert.match(index, new RegExp(`type="module" src="/src/${escapeRegExp(runtime)}`));
  }
});

test("v140 keeps the declared React owner so compatibility bridges cannot duplicate menus", () => {
  const runtime = read("src/studio-device-mode-v140.js");
  assert.match(runtime, /declaredOwner\.startsWith\("react-v"\)/);
  assert.doesNotMatch(runtime, /shell\.dataset\.navigationOwner = "react-v140";\n  shell\.dataset\.layoutAuthority/);
});
