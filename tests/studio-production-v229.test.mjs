...
test("v229 remains loaded after v228 even when a later authority exists", () => {
...
test("v229 preserves the native green reference map and all actionable areas", () => {
  for (const marker of [
  ...
  for (const marker of [
    "reference-blueprint-interactive",
    "below-map-full-width",
    "scaled-reference-small",
    "reference-large",
  ]) assert.ok(runtime.includes(marker), `v229 runtime missing ${marker}`);
...
test("twenty-six real widgets including custom HTML JavaScript remain preserved", () => {
...
test("theme editor keeps real line gutter and device-correct workspace geometry", () => {
...
test("sidebar and profile functional contracts remain available", () => {
...
test("Nara keeps Camera Photo File size model intelligence microphone and speaker contracts", () => {
  for (const marker of [
...
test("persistent auth and v229 compatibility cache remain without destructive session actions", () => {
