...
const isolation = read("src/studio-production-v224-action-isolation.js");
const isolationCss = read("src/studio-production-v224-action-isolation.css");
...
test("legacy v209 direct-button sweep cannot hide isolated HTML CSS JavaScript Theme actions", () => {
  ...
  assert.match(isolation, /v224-theme-code-actions/);
  assert.match(isolation, /data-v209-hidden-duplicate/);
  assert.match(isolationCss, /v224-theme-code-actions/);
  assert.match(runtime, /html-css-javascript-visible/);
  for (const label of ["Edit HTML", "Edit CSS", "Edit JavaScript"]) {
    assert.ok(isolation.includes(label) || runtime.includes(label), `missing isolated Theme label ${label}`);
  }
...
test("persistent authentication stays enabled and no destructive session action is introduced", () => {
  ...
  for (const source of [runtime, isolation, patch]) {
