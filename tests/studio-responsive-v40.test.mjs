...
test("v40 device authority loads last without replacing Nara or navigation", async () => {
  ...
  assert.doesNotMatch(runtime, /nara/i);
  assert.doesNotMatch(css, /\.nara-|\.sn-side/);
});

test("layout builder and published sites use genuinely different device geometry", async () => {
  const css = await read("src/studio-layout-device-v40.css");
...
test("site creation uses dynamic server capacity and hides a fixed marketing maximum", async () => {
...
test("Cloudflare deployment syncs domain bindings and validates readiness honestly", async () => {
