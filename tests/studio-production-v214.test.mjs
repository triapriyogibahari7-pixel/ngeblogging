...
const profileSourcePatch = read("scripts/patch-production-v214-profile-source.mjs");
const worker = read("public/sw.js");
const release = JSON.parse(read("public/release-v214.json"));
const RELEASE = "studio-production-v214-20260802";

test("v214 runs after v213 and remains a final extension instead of replacing proven authorities", () => {
...
test("six responsive engines keep phone app mobile compact small and tablet desktop family large", () => {
...
test("Theme code editor is 50:50 on large surfaces and preview above code on small surfaces", () => {
...
test("complete sidebar remains present and topbar Profile is separate from site Settings", () => {
  for (const label of [
  ...
  assert.match(studio, /ngeblogging:open-profile-v214/);
  assert.match(studio, /mode="profile"/);
  assert.match(studio, /mode="settings"/);
  assert.match(studio, /data-v214-settings-mode=\{mode\}/);
  assert.match(shellController, /studio-v214-separated-profile-controller/);
  assert.match(shellController, /CustomEvent\("ngeblogging:open-profile-v214"\)/);
  assert.match(profileCss, /data-v214-settings-mode="profile"/);
  assert.match(profileCss, /data-v214-settings-mode="settings"/);
});

test("100 real themes and 26 widgets remain synchronized including custom HTML JavaScript", () => {
...
test("Nara keeps small medium full, non-modal small medium, Camera Photo File, microphone speaker model intelligence", () => {
...
test("Domain and factual analytics receive readable mobile geometry without synthetic values", () => {
...
test("existing session recovery stack stays intact and v214 adds no synthetic site or destructive logout", () => {
  assert.match(auth, /persistSession:\s*true/);
  assert.match(auth, /autoRefreshToken:\s*true/);
  assert.match(gate, /degraded-session-retained/);
  assert.match(gate, /STUDIO_BOOTSTRAP_RECOVERY_V196/);
  assert.match(gate, /readPersistedSupabaseSessionV198/);
  assert.match(publicSite, /PUBLIC_SITE_SINGLE_RENDER_V209/);
  for (const source of [runtime, patch, profileSourcePatch]) {
...
test("release metadata stays factual and refuses unproven 100 percent or mass-login claims", () => {
  ...
  assert.equal(release.repairs.existingRetainedSessionRecoveryPreserved, true);
  assert.equal(release.repairs.newSyntheticSiteFallbackAdded, false);
  assert.equal(release.repairs.profileDropdownUsesStableV147Owner, true);
