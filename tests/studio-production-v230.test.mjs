...
test("v235 preempts old click authorities and makes screenshot interactions deterministic", () => {
  const runtime235 = read("src/studio-production-v235.js");
  const helper235 = read("src/studio-production-v235-widget-target.js");
  const css235 = read("src/studio-production-v235.css");
  const patch235 = read("scripts/patch-production-v235.mjs");
  const release235 = JSON.parse(read("public/release-v235.json"));
  const widgets = read("src/widget-system.js");
  const layoutRuntime = read("src/theme-layout-runtime-v170.js");

  assert.match(chain, /patch-production-v234\.mjs/);
  assert.match(chain, /patch-production-v235\.mjs/);
  assert.ok(chain.indexOf("patch-production-v234.mjs") < chain.indexOf("patch-production-v235.mjs"));
  assert.match(entry, /studio-production-v235\.js/);
  assert.match(entry, /studio-production-v235-widget-target\.js/);
  assert.ok(entry.indexOf("studio-production-v234.js") < entry.indexOf("studio-production-v235.js"));
  assert.match(runtime235, /studio-production-v235-interaction-map-nara-20260803/);
  assert.match(runtime235, /window\.addEventListener\("click"/);
  assert.match(runtime235, /stopImmediatePropagation/);
  assert.match(runtime235, /openProfileV178/);
  assert.match(runtime235, /v235-nara-attachment-portal/);
  assert.match(runtime235, /LEGACY_LAYOUT_LABEL/);
  assert.match(runtime235, /MAX_CODE_LINES = 10000/);
  assert.doesNotMatch(runtime235, /Array\.from\(\{ length: 10000 \}/);
  assert.match(helper235, /setReactSelect/);
  assert.match(helper235, /sidebar-right-4/);
  assert.match(css235, /data-v235-family="large"/);
  assert.match(css235, /data-v235-family="small"/);
  assert.match(css235, /v235-layout-popover/);
  assert.match(css235, /v235-code-gutter/);
  assert.match(css235, /v235-nara-attachment-portal/);
  assert.match(css235, /data-v235-domain-action/);
  assert.match(patch235, /ACTIVE_VERSION_V235/);
  assert.match(patch235, /ACTIVE_CACHE_RELEASE_V235/);
  assert.match(widgets, /sidebar-right-4/);
  assert.match(layoutRuntime, /sidebar-right-4/);
  assert.equal(release235.sidebar.singleInteractionAuthorityAtWindowCapture, true);
  assert.equal(release235.themeStudio.themeCount, 100);
  assert.equal(release235.themeStudio.widgetCount, 26);
  assert.equal(release235.themeStudio.layoutAreaRightFourthMadeRealAtBuild, true);
  assert.equal(release235.themeStudio.unconditionalTenThousandLineRenderingRemoved, true);
  assert.equal(release235.nara.attachmentMenuBodyPortal, true);
  assert.equal(release235.topbar.profileSeparatedFromSettings, true);
  assert.equal(release235.auth.automaticLogoutAdded, false);
  assert.equal(release235.validation.massCapacityClaimed, false);
  assert.doesNotMatch(runtime235, /localStorage\.clear\s*\(|sessionStorage\.clear\s*\(|signOut\s*\(/);
});
