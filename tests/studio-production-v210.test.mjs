...
const themePatch = read("scripts/patch-theme-layout-v210.mjs");
const productionPatch = read("scripts/patch-production-v210.mjs");
...
test("v210 is the final Studio and build authority after v209", () => {
  ...
  assert.ok(chain.indexOf("patch-production-v210.mjs") > chain.indexOf("patch-production-v209.mjs"));
  assert.ok(chain.indexOf("patch-production-v210.mjs") > chain.indexOf("patch-public-site-v209.mjs"));
...
test("existing authenticated accounts resume from current active-site snapshots", () => {
  ...
  for (const marker of ["snapshot-v209", "snapshot-v198", "snapshot-v195", "snapshot-v192", "snapshot-v191", "snapshot-v190"]) {
    assert.ok(fastGate.includes(marker), `fast gate missing ${marker}`);
  }
  assert.match(fastGate, /ACTIVE_SITE_STORAGE_KEY/);
  assert.doesNotMatch(fastGate, /signOut\s*\(|localStorage\.clear\s*\(|sessionStorage\.clear\s*\(/);
  assert.doesNotMatch(productionPatch, /localStorage\.clear\s*\(|sessionStorage\.clear\s*\(|signOut\s*\(/);
});
...
test("Theme build owns four sidebar slots on both sides and remembers clicked area", () => {
  ...
  assert.match(themeStudio, /data-theme-layout-v210="theme-layout-v210-20260802"/);
  assert.match(css, /\.sidebar-left-4/);
  assert.match(css, /\.sidebar-right-4/);
  assert.match(themePatch, /theme-layout-v210-20260802/);
});
...
test("advanced code editor is visible and split 50:50 on desktop while stacked on mobile", () => {
  ...
  assert.match(css, /\.tn-modal-layer>\.tn-modal/);
  ...
  for (const marker of ["id:\"html\",label:\"HTML\"", "id:\"css\",label:\"CSS\"", "id:\"javascript\",label:\"JavaScript\"", "PREVIEW LANGSUNG"]) {
    assert.ok(themeStudio.includes(marker), `code editor missing ${marker}`);
  }
});
...
test("public site single initial render and safe cache rotation are preserved", () => {
  const publicSite = read("src/PublicSiteNext.jsx");
  const worker = read("public/sw.js");
  assert.match(publicSite, /PUBLIC_SITE_SINGLE_RENDER_V209/);
  assert.match(productionPatch, /ngeblogging-app-v210-theme-nara-domain-mobile-20260802/);
  assert.match(productionPatch, /theme-nara-domain-mobile-cache-v210/);
  assert.match(worker, /STUDIO_PRODUCTION_RELEASE_V210/);
  assert.match(worker, /ngeblogging-app-v209-theme-domain-nara-20260802/);
