...
const MOBILE_CLASSIFIER_FIX_V294 = "studio-mobile-classifier-v294-20260805";
...
function rawDesktopSiteRequested(view, handheld) {
  ...
  // Mobile Chrome may report a desktop-like width briefly before viewport-fit
  // settles. A browser that still declares itself mobile must not be promoted
  // to the desktop family from that transient measurement.
  ...
  // Preserve the v267 compatibility marker while requiring an explicit
  // desktop-UA signal. A wide Android touch surface by itself is not desktop.
...
function desktopSiteRequested(view, handheld) {
  ...
  // Returning to an explicitly mobile UA releases the desktop-site lock
  // immediately. A real desktop-site request remains sticky through address-bar,
  // keyboard, zoom and visualViewport changes.
...
function applyDeviceMode() {
  ...
  root.dataset.studioMobileClassifierV294 = MOBILE_CLASSIFIER_FIX_V294;
  ...
  if (lastSignature !== signature) {
    ...
    window.dispatchEvent(new CustomEvent(MODE_EVENT, {
      detail: {
        ...
        release: RELEASE,
        classifierRelease: MOBILE_CLASSIFIER_FIX_V294,
        handheld,
        desktopSitePhone,
        ...view,
      },
