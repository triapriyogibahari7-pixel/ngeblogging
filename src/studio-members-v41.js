...
async function membersData(siteId) {
  ...
  const [{ data:members, error:memberError }, { data:invitations, error:inviteError }, { data:quota, error:quotaError }] = await Promise.all([
  ...
  if (profileResult.error && profileResult.error.code !== "42501") throw profileResult.error;
...
function markup(data, inviteReady) {
  const quota = data.quota || {
    active_count:data.members.length,
    pending_count:data.invitations.length,
    allowed_limit:100,
    remaining:Math.max(100-data.members.length-data.invitations.length,0),
    can_invite:false,
  };
  return `<section class="op41-panel">
    ...
    <div class="op41-member-grid"><section class="op41-card"><header><div><small class="op41-kicker">TIM AKTIF</small><h2>${formatNumber(data.members.length)} anggota</h2></div></header><div class="op41-list">${data.members.map((member) => { const name=member.profile?.display_name || "Pengguna"; return `<article><span>${escapeHtml(name.slice(0,2).toUpperCase())}</span><div><b>${escapeHtml(name)}</b><small>Bergabung ${formatDate(member.joined_at)}</small></div><i class="op41-role">${escapeHtml(ROLE_LABEL[member.role] || member.role)}</i></article>`; }).join("") || `<p>Belum ada anggota yang dapat ditampilkan.</p>`}</div></section><section class="op41-card"><header><div><small class="op41-kicker">UNDANGAN AKTIF</small><h2>${formatNumber(data.invitations.length)} menunggu</h2></div></header><div class="op41-list">${data.invitations.map((invite) => `<article data-invitation-id="${escapeHtml(invite.id)}"><div><b>${escapeHtml(invite.email)}</b><small>${escapeHtml(ROLE_LABEL[invite.role] || invite.role)} · berakhir ${formatDate(invite.expires_at)}</small></div>${inviteReady ? `<button type="button" class="op41-button" data-cancel-invite>Batalkan</button>` : ""}</article>`).join("") || `<p>Tidak ada undangan yang menunggu.</p>`}</div></section></div>
  </section>`;
}
