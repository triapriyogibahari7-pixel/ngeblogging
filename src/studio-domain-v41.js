...
function domainView() {
  return [...document.querySelectorAll(".sn-main > .sn-view-pad")].find((view) => view.querySelector(":scope > .sn-page-title h1")?.textContent?.trim() === "Domain & publikasi") || null;
}

async function health() {
  if (healthCache && Date.now() - healthAt < 10_000) return healthCache;
  healthCache = await fetch("/api/health", { cache: "no-store", headers: { accept: "application/json", "cache-control": "no-cache" } }).then((response) => response.ok ? response.json() : {}).catch(() => ({}));
  healthAt = Date.now();
  return healthCache;
}
...
function readinessRows(state) {
  ...
  return [["Token Cloudflare", bindings.apiToken], ["Zone ngeblogging.com", bindings.zoneId], ["Target CNAME resmi", bindings.cnameTarget], ["Penyimpanan JWT + RLS", bindings.databaseAccess]].map(([label, ready]) => `<li data-ready="${ready === true}">${ready === true ? "✓" : "○"} ${label}</li>`).join("");
}
