const RELEASE = "studio-shell-v29-20260725";
...
function syncRoot(profile) {
  const root = document.documentElement;
  root.dataset.studioShellAuthorityV29 = RELEASE;
  root.dataset.studioShellModeV29 = profile.device;
  root.dataset.studioShellCompactV29 = String(profile.compact);
  for (const name of ["phone", "mobile", "app", "tablet", "desktop-phone", "laptop", "desktop"]) {
    root.classList.toggle(`studio-v29-${name}`, profile.device === name);
  }
  root.classList.toggle("studio-v29-compact", profile.compact);
  root.classList.toggle("studio-v29-standalone", profile.standalone);
}
...
function ensureMobileHeader(shell, side) {
  let header = side.querySelector(":scope > .sn-mobile-v29-header");
  if (!header) {
    header = document.createElement("header");
    header.className = "sn-mobile-v29-header";
    header.innerHTML = `<div class="sn-mobile-v29-brand"><strong>n<span>.</span></strong><b>ngeblogging</b></div><button type="button" class="sn-mobile-v29-close" aria-label="Tutup menu">${CLOSE_ICON}</button>`;
    header.querySelector("button")?.addEventListener("click", () => clickSource(shell, false));
    side.prepend(header);
  }

  let search = side.querySelector(":scope > .sn-mobile-v29-search");
  if (!search) {
    search = document.createElement("label");
    search.className = "sn-mobile-v29-search";
    ...
    input?.addEventListener("input", () => {
      ...
      side.querySelectorAll(":scope > nav > button").forEach((button) => {
        button.dataset.v29SearchHidden = String(Boolean(query) && !buttonLabel(button).toLocaleLowerCase("id-ID").includes(query));
      });
    });
    const logo = side.querySelector(":scope > .sn-logo");
    logo?.insertAdjacentElement("afterend", search);
    if (!logo) header.insertAdjacentElement("afterend", search);
  }
}

function ensureMobileLauncher(shell) {
  let launcher = shell.querySelector(":scope > .sn-mobile-v29-launcher");
  if (!launcher) {
    launcher = document.createElement("button");
    launcher.type = "button";
    launcher.className = "sn-mobile-v29-launcher";
...
function ensureMobileScrim(shell) {
  let scrim = shell.querySelector(":scope > .sn-mobile-v29-scrim");
  if (!scrim) {
    scrim = document.createElement("button");
    scrim.type = "button";
    scrim.className = "sn-mobile-v29-scrim";
...
function removeLegacySidebarChrome(shell) {
  shell.querySelectorAll(":scope > .sn-device-toggle-v26, :scope > .sn-device-toggle-v27, :scope > .sn-device-scrim-v27, :scope > .sn-sidebar-scrim-v23")
...
function syncSidebar(shell, profile) {
  ...
  removeLegacySidebarChrome(shell);
  ...
  source.hidden = false;
  source.disabled = false;
  source.dataset.v29SourceToggle = profile.compact ? "programmatic" : "visible";
  ...
  shell.dataset.v29SidebarOpen = String(open);
  ...
  launcher.setAttribute("aria-controls", side.id || "ngeblogging-studio-sidebar-v29");

  scrim.hidden = !profile.compact || !open;
  scrim.disabled = !profile.compact || !open;
  scrim.tabIndex = profile.compact && open ? 0 : -1;
  side.id ||= "ngeblogging-studio-sidebar-v29";
...
function setNaraSize(layer, size) {
  ...
  state.size = resolved;
  layer.dataset.naraShellV29 = "true";
  layer.dataset.naraSizeV29 = resolved;
  layer.dataset.naraWindowMode = resolved === "expanded" ? "expanded" : "compact";
  shell.dataset.naraControlsV29 = "true";
  shell.dataset.naraSizeV29 = resolved;
  shell.dataset.naraWindowMode = layer.dataset.naraWindowMode;
  document.documentElement.dataset.naraV29Open = "true";

  const sizeButton = shell.querySelector(".nara-size-toggle-v29");
  if (sizeButton) {
  ...
  const expandButton = shell.querySelector(".nara-expand-toggle-v29");
  if (expandButton) {
...
function ensureSpeaker(shell) {
  ...
  if (!tools) return;
  let button = tools.querySelector(":scope > .nara-speaker-v29");
  if (!button) {
    button = document.createElement("button");
    button.type = "button";
    button.className = "nara-speaker-v29";
    ...
    const mic = [...tools.querySelectorAll(":scope > button")].find((candidate) => /suara/i.test(candidate.title || candidate.getAttribute("aria-label") || ""));
...
function ensureTaskMode(shell) {
  const tools = shell.querySelector(".nara-composer-tools");
  if (!tools || tools.querySelector(":scope > .nara-mode-v29")) return;
  const label = document.createElement("label");
  label.className = "nara-mode-v29";
  ...
  select.addEventListener("change", () => {
    ...
    shell.dataset.naraTaskModeV29 = select.value;
  });
...
function ensureNaraControls(shell, profile) {
  ...
  header.querySelectorAll(":scope > .nara-size-toggle-v26, :scope > .nara-size-toggle-v27, :scope > .nara-window-toggle-v24")
    .forEach((node) => node.remove());
  ...
  let expand = header.querySelector(":scope > .nara-expand-toggle-v29");
  if (!expand) {
    expand = document.createElement("button");
    expand.type = "button";
    expand.className = "nara-expand-toggle-v29";
  ...
  let size = header.querySelector(":scope > .nara-size-toggle-v29");
  if (!size) {
    size = document.createElement("button");
    size.type = "button";
    size.className = "nara-size-toggle-v29";
  ...
  const state = naraProfile(shell, profile);
  if (!layer.dataset.naraInitialV29) {
    layer.dataset.naraInitialV29 = "true";
...
function autoOpenNara(profile) {
  ...
  document.documentElement.dataset.naraV29Open = "false";
  const launcher = document.querySelector(".nara-floating-button");
  if (!launcher || launcher.dataset.autoOpenedV29 === "true") return;
  launcher.dataset.autoOpenedV29 = "true";
...
document.addEventListener("keydown", (event) => {
  ...
  if (profile.compact && shell && side && !side.classList.contains("collapsed")) {
    clickSource(shell, false);
    return;
  }
  const naraLayer = document.querySelector('.nara-assistant-layer[data-nara-shell-v29="true"]');
  if (!naraLayer) return;
  const naraShell = naraLayer.querySelector(":scope > .nara-assistant-shell");
  const state = naraShell ? naraProfile(naraShell, profile) : null;
  if (state?.size === "expanded") setNaraSize(naraLayer, state.previous);
  else if (state?.size === "compact") setNaraSize(naraLayer, "mini");
});
