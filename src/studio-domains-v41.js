...
function dnsRows(domain, cnameTarget) {
  const rows = [];
  if (cnameTarget) rows.push({
    label:"1 · Arahkan trafik",
    type:"CNAME",
    name:domain.hostname,
    value:cnameTarget,
    note:"Gunakan @ bila panel DNS meminta nama host singkat untuk domain utama.",
  });
  ...
  if (ownership.name && ownership.value) rows.push({
    ...
    note:"Catatan resmi untuk membuktikan kepemilikan domain.",
  });
  for (const record of Array.isArray(domain.ssl_validation) ? domain.ssl_validation : []) {
    ...
    if (name && value) rows.push({
      label:"Tambahan · Validasi HTTPS",
      type:record.type || (record.cname ? "CNAME" : "TXT"),
      name,
      value,
      note:"Hanya muncul bila sertifikat HTTPS memerlukan validasi tambahan.",
    });
  }
  return rows;
}
...
function domainItems(domains, cnameTarget) {
  ...
  return domains.map((domain) => {
    ...
    return `<article class="op41-domain" data-domain-id="${escapeHtml(domain.id)}"><header><div><small class="op41-kicker">CUSTOM DOMAIN</small><h3>${escapeHtml(domain.hostname)}</h3></div><i class="op41-domain-status${active ? " active" : ""}">${active ? "Aktif" : "Perlu verifikasi"}</i></header><div class="op41-dns-wrap"><div class="op41-dns">${dnsRows(domain,cnameTarget).map((row) => `<div><span><b>${escapeHtml(row.label)}</b><small>${escapeHtml(row.note)}</small></span><code>${escapeHtml(row.type)}</code><code>${escapeHtml(row.name)}</code><code>${escapeHtml(row.value)}</code><button type="button" class="op41-button" data-copy="${escapeHtml(row.value)}">Salin</button></div>`).join("") || `<p>Cloudflare sedang menyiapkan catatan DNS.</p>`}</div></div><footer><a class="op41-button" href="https://${escapeHtml(domain.hostname)}" target="_blank" rel="noreferrer">Buka domain</a><button type="button" class="op41-button" data-domain-action="refresh">Periksa status</button><button type="button" class="op41-button" data-domain-action="remove">Hapus</button></footer></article>`;
  }).join("");
}
...
function readinessMarkup(state) {
  ...
  return `<section class="op41-readiness"><div><small class="op41-kicker">CUSTOM DOMAIN</small><h2>Koneksi produksi sedang diverifikasi</h2><p>Form domain hanya dibuka setelah seluruh binding server terbaca oleh Worker. Subdomain gratis tetap aktif. Tekan periksa ulang setelah deployment selesai.</p><button type="button" class="op41-button primary op41-domain-retry">Periksa ulang</button></div><ul>${rows.map(([name, ready]) => `<li data-ready="${ready === true}">${ready === true ? "✓" : "○"} ${name}</li>`).join("")}</ul></section>`;
}
