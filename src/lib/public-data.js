...
export async function resolvePublishedSite({ slug = "", hostname = "" }) {
  ...
  if (hostname && !hostname.endsWith(".ngeblogging.com")) {
    const { data: domain, error: domainError } = await withTimeout(
      db.from("site_domains").select("site_id").eq("hostname", hostname.toLowerCase()).eq("status", "active").maybeSingle(),
  ...
  if (!site) return null;
  const { data: theme, error: themeError } = await withTimeout(
    db.from("site_theme_settings").select("active_theme_id,published_config,code,widgets,updated_at").eq("site_id", site.id).maybeSingle(),
    "Memuat tema situs",
  );
...
export async function getPublishedContent(siteId, slug) {
  const { data, error } = await withTimeout(
    client().from("contents").select("id,kind,title,slug,body_html,excerpt,featured_image_path,metadata,seo,published_at,updated_at,created_at").eq("site_id", siteId).eq("slug", slug).eq("status", "published").eq("visibility", "public").maybeSingle(),
