...
const RELEASE = "auth-callback-authority-v141-20260729";
const GATE_ID = "ngeblogging-auth-callback-gate-v141";
const CALLBACK_MARKER = "ngeblogging-auth-callback-v141";
const PASSWORD_PATCH = Symbol.for("ngeblogging.auth.passwordFallbackV141");
...
function installGate(mode) {
  ...
  Object.assign(spinner.style, {
    ...
    animation: "ngeblogging-callback-spin-v141 .75s linear infinite",
  });
  ...
  style.textContent = "@keyframes ngeblogging-callback-spin-v141{to{transform:rotate(360deg)}}";
  style.dataset.authCallbackV141 = "true";
...
function successTarget(mode) {
  ...
  url.searchParams.set("auth_success", "v141");
...
async function directPasswordGrant(credentials) {
  ...
  const response = await fetch(`${SUPABASE_URL}/auth/v1/token?grant_type=password`, {
    ...
    headers: {
      ...
      "x-client-info": "ngeblogging-auth-v141",
    },
    body: JSON.stringify({
...
function installPasswordFallback() {
  ...
  supabase.auth.signInWithPassword = async (credentials) => {
    ...
    try {
      ...
      document.documentElement.dataset.authPasswordTransportV141 = "direct-recovery";
      return recovered;
    } catch (fallbackError) {
      document.documentElement.dataset.authPasswordTransportV141 = "failed";
  ...
  document.documentElement.dataset.authPasswordFallbackV141 = "installed";
}

function publishSession(mode, session, state) {
  window.__ngebloggingOAuthCallbackSessionV141 = session;
  document.documentElement.dataset.authCallbackV141 = state;
  try {
...
async function completeCallback(mode, code) {
  ...
  try {
    ...
    console.error("OAuth callback v141 failed", initialError);
    document.documentElement.dataset.authCallbackV141 = "failed";
